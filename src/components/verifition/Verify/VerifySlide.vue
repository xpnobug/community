...
export default {
  ...
  setup(props, context) {
    ...
    function getPictrue() {
      ...
      reqGet(type).then(res => {
        const resDate = res.data.data;
        if (res.status === 200) {
          backImgBase.value = resDate.originalImageBase64
          blockBackImgBase.value = resDate.jigsawImageBase64
          backToken.value = resDate.token
          secretKey.value = resDate.secretKey
        } else {
          tipWords.value = resDate.repMsg;
        }
      })
    }
