      ...
      try {
        ...
        if (rtc) {
          ...
          if (!offerSdp) {
            ...
            message.error('webRtcSrs的offerSdp为空');
            return;
          }
          await rtc.setLocalDescription(offerSdp!);
          const userId = userStore.userInfo?.userId;
          const liveRooms = await fetchUserHasLiveRoom(userId);
          const myLiveRoom = liveRooms.data.data.live_room;
          console.log(myLiveRoom,"live_roomlive_room")
          if (!myLiveRoom) {
          ...
          const answerRes = await fetchRtcV1Publish({
            ...
            clientip: null,
            sdp: offerSdp.sdp!,
            streamurl: `${myLiveRoom.rtmpUrl!}?${SRS_CB_URL_PARAMS.publishKey}=${myLiveRoom.secretKey!}&${SRS_CB_URL_PARAMS.publishType}=${
