...
import {useFlvPlay, useHlsPlay} from '@/hooks/live/use-play';
import {useWebsocket} from '@/hooks/live/use-websocket';
import {
  DanmuMsgTypeEnum,
  LiveLineEnum,
  LiveRenderEnum,
...
import {ILiveRoom, LiveRoomTypeEnum, LiveRoomUseCDNEnum,} from '@/api/live/types/ILiveRoom';
import {WsMessageType, WsMsgTypeEnum} from '@/api/live/types/websocket';
...
export function usePull(roomId: string) {
  ...
  function handlePlay(data: ILiveRoom) {
    roomLiving.value = true;
    flvurl.value =
        data.cdn === LiveRoomUseCDNEnum.yes ? data.cdnFlvUrl! : data.flvUrl!;
    hlsurl.value =
        data.cdn === LiveRoomUseCDNEnum.yes ? data.cdnHlsUrl! : data.hlsUrl!;
