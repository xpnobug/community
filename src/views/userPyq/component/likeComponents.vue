  ...
  <footer class="post-footer g-clear-both">
    ...
    <div class="post-fun g-right">
      ...
      <div :id="prop.postInfo.articleId" class="fun-box">
        <div :class="isLikedByUser ? 'fun-btn like allow liked':'fun-btn like allow'" :data-index="prop.index"
             data-action="like"
  ...
  <aside :id="'like-'+prop.postInfo.articleId" class="post-aside show">
    ...
    <div v-show="isLike" :id="'post-'+prop.postInfo.articleId+'-mxLp-like'"
      ...
      <ul class="like-userslist g-right-flex">
        <!-- 遍历 likeList，显示每个点赞用户 -->
        <li v-for="item in prop.likeList" v-show="item.articleId === prop.postInfo.articleId"
            :id="'post-'+prop.postInfo.articleId+'-like-me'"
            :key="item.likeId"
            :class="item.userId !== null ? 'like-name g-user-select' : 'like-name more'"
            data-separator=",">
          {{ item.userId === null ? guestCount + '位访客' : item.userName }}
        </li>
      </ul>
    </div>
...
const guestCount = computed(() => {
  let count = 0;
  prop.likeList.forEach(item => {
    if (item.userId === null && item.articleId === prop.postInfo.articleId) {
      count++;
    }
  });
  return count;
...
const isLikedByUser = computed(() => {
  return prop.likeList.some(item => item.userId === userId && item.articleId === prop.postInfo.articleId);
});
