<template>
  <arco-modal v-model:visible="visible" width="90%" :footer="false">
    <template #title>
      {{ `生成 ${previewTableName} 表预览` }}
      <arco-link style="margin-left: 10px" icon @click="onDownload">下载源码</arco-link>
    </template>
    <div class="preview-content">
      <arco-layout :has-sider="true">
        <arco-layout-sider theme="dark" style="max-width:600px; height: 700px"
                           :resize-directions="['right']" :width="370">
          <arco-tree
              ref="treeRef"
              :data="treeData"
              show-line
          ...
          >
            ...
            <template #icon="node">
              ...
              <GiSvgIcon v-if="!node.isLeaf && node.expanded" :size="16"
                         name="directory-open-blue"/>
              <GiSvgIcon v-if="node.isLeaf && checkFileType(node.node.title, '.java')" :size="16"
                         name="file-java"/>
              <GiSvgIcon v-if="node.isLeaf && checkFileType(node.node.title, '.vue')" :size="16"
                         name="file-vue"/>
              ...
              <GiSvgIcon v-if="node.isLeaf && checkFileType(node.node.title, '.json')" :size="16"
                         name="file-json"/>
              <GiSvgIcon v-if="node.isLeaf && checkFileType(node.node.title, 'pom.xml')" :size="16"
                         name="file-maven"/>
              <GiSvgIcon
                  v-if="node.isLeaf && checkFileType(node.node.title, '.xml') && !checkFileType(node.node.title, 'pom.xml')"
                  :size="16" name="file-xml"/>
              <GiSvgIcon v-if="node.isLeaf && checkFileType(node.node.title, '.sql')" :size="16"
                         name="file-sql"/>
            </template>
          </arco-tree>
        </arco-layout-sider>
        <arco-layout-content>
          <arco-card>
            <template #title>
              <arco-typography-title :heading="6" ellipsis>
                {{ currentPreview?.path }}{{
                  currentPreview?.path.indexOf('/') !== -1 ? '/' : '\\'
                }}{{ currentPreview?.fileName }}
              </arco-typography-title>
            </template>
            <arco-scrollbar style="height: 650px; overflow: auto">
              <arco-link style="position: absolute; right: 20px; z-index: 999" @click="onCopy">
                <template #icon>
...
const onPreview = async (tableName: string) => {
  ...
  genPreviewList.value = res.data.data
  ...
  selectedKeys.value = [genPreviewList.value[0].fileName]  // 确保是数组
...
const onSelectPreview = (keys: (string | number)[]) => {
  if (typeof keys[0] === 'string') {
    ...
    selectedKeys.value = [keys[0] as string]  // 确保是数组
  } else {
...
.preview-content :deep(.arco-layout-sider) {
  min-width: 200px;
  white-space: nowrap;
  background-color: var(--reaicc-meta-theme-post-color)!important;
}
