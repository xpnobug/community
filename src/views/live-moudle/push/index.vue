...
import {liveRoomTypeEnumMap, mediaTypeEnumMap, QINIU_RESOURCE, THEME_COLOR,} from '@/constant';
import {emojiArray} from '@/emoji';
import {commentAuthTip, loginTip} from '@/hooks/live/use-login';
import {usePush} from '@/hooks/live/use-push';
...
import {WsMsgTypeEnum, WsUpdateLiveRoomCoverImg} from '@/api/live/types/websocket';
import {
  base64ToFile,
  createVideo,
  formatDownTime2,
...
function renderFrame() {
  /**
   * 理论情况：
   * 如果 currentMaxFramerate 等于 20，则每秒20帧，即1000 / 20 = 50毫秒轮询一次。
   * 如果 currentMaxFramerate 等于 30，则每秒30帧，即1000 / 30 = 33.333毫秒轮询一次。
   * 如果 currentMaxFramerate 等于 60，则每秒60帧，即1000 / 60 = 16.666毫秒轮询一次。
   *
   * 实际情况：
   * 如果 currentMaxFramerate 等于 20，按50毫秒轮询一次，实际fps是18。
   * 希望实际fps达到20，则需要调整轮询间隔为 (18/20) * 50 = 45毫秒。
   */
  let delay = 1000 / currentMaxFramerate.value; // 计算理论轮询间隔时间
  delay = (18 / 20) * delay; // 根据实际FPS调整轮询间隔时间

  // 向 Worker 发送停止当前循环的请求
  worker.value?.postMessage({
    type: 'request-clear-loop',
    timer: workerTimerId.value,
  });

  // 向 Worker 发送启动新循环的请求，并传递调整后的轮询间隔时间
  worker.value?.postMessage({
  ...
  worker.value?.addEventListener('message', (e) => {
    if (e.data.type === 'response-start-loop') {
      workerTimerId.value = e.data.timer; // 存储 Worker 返回的计时器ID
...
function handleMixedAudio() {
  ...
  console.log('allAudioTrack', allAudioTrack);
  // 获取占位空音频的音频轨道
  ...
  // 遍历 networkStore.rtcMap 中的每个 RTC 对象
  networkStore.rtcMap.forEach((rtc) => {

    // 获取与指定音频轨道 ID (resAudio.id) 匹配的 sender
    const sender = rtc.peerConnection
        ?.getSenders() // 获取发送者
        .find((sender) => sender.track?.id === resAudio.id); // 查找音轨 ID 匹配的发送者

    // 如果未找到匹配的 sender，则替换第一个音频轨道
    if (!sender) {
      rtc.peerConnection
          ?.getSenders() // 获取所有发送者
          ?.find((sender) => sender.track?.kind === 'audio') // 查找第一个音频轨道的发送者
          ?.replaceTrack(resAudio); // 用新的音频轨道替换
    }
  });
...
async function handleRecordVideo() {
  ...
  try {
    if (!recording.value) {
      ...
      message.success(`录制文件: ${suggestedName.value} 已保存到本地`);
      avRecorder = null;
    }
...
async function uploadLivePreview() {
  ...
  formData.append('formDate', file); // 确保文件正确添加到 FormData 中
  ...
  if (uploadRes.status === 200) {
    networkStore.wsMap
        ...
        ?.send<WsUpdateLiveRoomCoverImg['data']>({
          ...
          data: {
            coverImg: uploadRes.data.data.downloadUrl,
          },
        });
  }
}
...
function autoCreateVideo(data) {
  ...
  return new Promise((resolve) => {
    videoEl.onloadedmetadata = () => {
      ...
      function main() {
        const width =
            stream?.getVideoTracks()[0].getSettings().width || videoEl.videoWidth;
        const height =
            stream?.getVideoTracks()[0].getSettings().height || videoEl.videoHeight;
...
async function handleCache() {
  ...
  cacheStore['resource-list'].forEach((item) => {
    console.log(item, "navigator.mediaDevices.getUserMedia")
    // @ts-ignore
    ...
    async function handleMediaVideo() {
      ...
      console.log("code ,file", code, file)
      if (code === 1 && file) {
...
function handleShowMediaModalCpt(val: MediaTypeEnum) {
  ...
  console.log(val,' v-if="showSelectMediaModalCpt"')
}
