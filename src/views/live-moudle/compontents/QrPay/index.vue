...
import { fetchAliPay, fetchAliPayStatus } from '@/api/live/order';
import { PayStatusEnum } from '@/api/live/interface';
...
async function handleStartPay() {
  ...
  try {
    ...
    if (res.data.code === 200) {
      if (isMobile()) {
        hrefToTarget(res.data.data.qr_code);
        return;
      }
      const base64 = await generateQR(res.data.data.qr_code);
      aliPayBase64.value = base64;
      getPayStatus(res.data.data.out_trade_no);
      handleDownTime();
    }
  } catch (error) {
    console.log(error);
