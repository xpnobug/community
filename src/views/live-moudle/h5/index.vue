        ...
        <div class="live-room-list">
          ...
          >
            ...
            >
              <PullAuthTip
                v-if="
                  iten?.live_room?.pullIsShouldAuth ===
...
import { fetchAreaLiveRoomList } from '@/api/live/area';
import { QINIU_RESOURCE } from '@/constant';
import { IArea, IAreaLiveRoom } from '@/api/live/interface';
...
async function getLiveRoomList() {
  try {
    ...
    if (res.data.code === 200) {
      liveRoomList.value = res.data.data.rows;
    }
  } catch (error) {
    console.log(error);
  }
