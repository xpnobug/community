<template>
  <div class="gi-table" :class="{ 'gi-table--fullscreen': isFullscreen }">
    <arco-row v-if="props.title" justify="space-between" align="center" class="gi-table__header">
      <arco-space wrap>
    ...
    <arco-row justify="space-between" align="center" class="gi-table__toolbar">
      <arco-space wrap class="gi-table__toolbar-left" :size="[8, 8]">
        <slot name="custom-left"></slot>
      </arco-space>
      <arco-space wrap class="gi-table__toolbar-right" :size="[8, 8]">
        <slot name="custom-right"></slot>
        <arco-tooltip content="刷新">
          <arco-button v-if="showRefreshBtn" class="gi_hover_btn-border" @click="refresh">
            <template #icon><icon-refresh /></template>
          </arco-button>
        </arco-tooltip>
        <arco-dropdown v-if="showSizeBtn" @select="handleSelect">
          <arco-tooltip content="尺寸">
            <arco-button class="gi_hover_btn-border">
              <template #icon><icon-table-size style="width: 14px; height: 14px" /></template>
            </arco-button>
          </arco-tooltip>
          <template #content>
            <arco-doption v-for="item in sizeList" :key="item.value" :value="item.value" :active="item.value === size">
              {{
                item.label }}
            </arco-doption>
          </template>
        </arco-dropdown>
        <arco-popover v-if="showSettingColumnBtn" trigger="click" position="br"
          :content-style="{ minWidth: '120px', padding: '6px 8px 10px' }">
          <arco-tooltip content="列设置">
            <arco-button class="gi_hover_btn-border">
              <template #icon>
                <icon-settings />
              </template>
            ...
            <div class="gi-table__draggable">
              <VueDraggable v-model="settingColumnList">
                <div v-for="item in settingColumnList" :key="item.title" class="drag-item">
                  ...
                  <arco-checkbox v-model:model-value="item.show" :disabled="item.disabled">{{ item.title }}</arco-checkbox>
                </div>
              </VueDraggable>
            </div>
            <arco-divider :margin="6" />
            <arco-row justify="center">
              <arco-button type="primary" size="mini" long @click="resetSettingColumns">
                <template #icon><icon-refresh /></template>
        ...
        <arco-tooltip content="全屏">
          <arco-button v-if="showFullscreenBtn" class="gi_hover_btn-border" @click="isFullscreen = !isFullscreen">
            <template #icon>
    ...
    <div class="gi-table__body" :class="`gi-table__body-pagination-${attrs['page-position']}`">
      <div class="gi-table__container">
        <arco-table ref="tableRef" :stripe="stripe" :size="size" column-resizable :bordered="{ cell: isBordered }"
      ...
      <template v-if="attrs.pagination">
        ...
        <slot name="custom-pagination" :pagination-props="attrs.pagination">
          <arco-pagination v-bind="attrs.pagination" />
        </slot>
