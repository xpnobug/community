<template>
  <div>
    <!-- 按钮：显示模态框 -->
    <a-button type="primary" @click="showModal">创建用户版块</a-button>
    ...
    >
      ...
      <!-- 模态框标题 -->
      <template #title>
        <div ref="modalTitleRef" style="width: 100%; cursor: move">创建用户版块</div>
      </template>
      <!-- 模态框自定义渲染 -->
...
const props = defineProps<{
  getUserChannelData: () => Promise<void>;
}>();
...
const handleSubmit = (formState: any, rawFormState: any) => {
  ...
  //用户
  formState.isOfficial = 2
  addChannel(formState).then(async res => {
    if (res.status == 200) {
      message.success("保存成功");
      await props.getUserChannelData();
    }
  })
  open.value = false;
};
